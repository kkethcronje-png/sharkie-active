...
export default function ProductCard({ product, addToCart }) {
  return (
    <motion.div whileHover={{ scale: 1.03 }} transition={{ type: 'spring', stiffness: 300 }} className="group">
      ...
      <div className="mt-4 w-full py-3 border border-charcoal text-center text-sm tracking-wide rounded-full text-charcoal">COMING SOON</div>
    </motion.div>
  );
}
...
export default function Home() {
  ...
  return (
    <>
      <div className="fixed top-0 w-full z-50 bg-black text-offwhite text-center text-xs tracking-widest py-2">SHARKIE ACTIVE — PREVIEW BUILD</div>
      <Header cartCount={0} />
      <motion.section className="relative h-screen flex items-center justify-center bg-black text-offwhite" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
        ...
        <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3, duration: 0.8 }} className="relative z-10 text-center px-6">
          <h1 className="text-5xl md:text-7xl font-semibold tracking-[0.15em]">SHARKIE ACTIVE</h1>
          <p className="mt-6 text-base md:text-lg text-charcoal max-w-xl mx-auto uppercase tracking-widest">Engineered for movement. Designed for confidence.</p>
        </motion.div>
      </motion.section>
